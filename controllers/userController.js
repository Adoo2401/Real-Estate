  ...
  try {
    ...
    if (!validator.isLength(password, { min: 8, max: 15 })) {
      return responseSend(
        ...
        500,
        false,
        "Password Should be minimun 8 characters and maximun 15"
      );
    }
  ...
  try {
    if (req.query.isGoogle) {
      ...
      const user = await User.findOne({
        $or: [{ email: emailRegex }, { phone: req.body.email }],
      })
        .select("+password")
        .clone();
...
exports.addFavourite = async (req, resp) => {
  try {
    ...
    let check = user.favourites.find((elm) => elm == property_id.toString());
    ...
    if (check) {
      return responseSend(
        ...
        500,
        false,
        "Property already added to favourite list"
      );
    }
...
exports.getFavourites = async (req, resp) => {
  try {
    ...
    for (var i = 0; i < user.favourites.length; i++) {
      let prop = await Property.findById(user.favourites[i]);
      fav.push(prop);
    }
...
exports.recent = async (req, resp) => {
  try {
    ...
    let check = user.recent.filter(
      (elm) => elm.toString() === req.params.propertyId
    );

    if (check.length > 0) {
      return resp
        .status(422)
        .json({ success: false, message: "Only in recents of user" });
    }
    ...
    if (user.recent.length == 5) {
      ...
      return resp
        .status(201)
        .json({ success: true, message: "Added in recents of user" });
    }
  ...
  try {
    ...
    await User.findByIdAndUpdate(req.user._id, {
      $set: {
        ...
        "setting.featuredAd": featuredAd,
        "setting.viewAd": viewAd,
      },
    });
...
exports.getUnseenNotification = async (req, resp) => {
  try {
    let notification = await User.find({
      _id: req.user._id,
